...
LinearAlgebra::cosine_similarity(const std::vector<std::vector<double>>& a, 
    ...
    std::vector<double> similarities(N);

    for (size_t i = 0; i < N; i++) {
        const double* ai = a[i].data();
        const double* bi = b[i].data();

        double dotProduct = cblas_ddot(D, ai, 1, bi, 1);

        double a_norm = cblas_dnrm2(D, ai, 1);
        double b_norm = cblas_dnrm2(D, bi, 1);

        similarities[i] = dotProduct / (std::max(a_norm,eps) * std::max(b_norm,eps));
    }
